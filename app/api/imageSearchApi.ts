  ...
  export const searchImages = async (query: string): Promise<ImageResult[] | null> => {
    try {
      ...
      if (!apiKey || !searchEngineId) {
        ...
        alert('Please set your API key and Search Engine ID in the environment variables.');
        return null;
      }
      ...
      if (!response.ok) {
        ...
        alert(`Error fetching images: Status ${response.status} - ${response.statusText || "No status text provided"}`);
        return null;
      }
